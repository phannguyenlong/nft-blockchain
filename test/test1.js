...
async function submitConfig(originalBlock, modifiedBlock, channelName) {
    ...
    shell.env["PATH"] = __dirname + `` + shell.env["PATH"] // set path to bin
    // calculate the delta between these two config protobufs
    shell.exec(`configtxlator compute_update --channel_id ${channelName} --original ${NETWORK_PATH}/channel-artifacts/${channelName}/${originalBlock}.pb --updated ${NETWORK_PATH}/channel-artifacts/${channelName}/${modifiedBlock}.pb --output config_update.pb`)
...
async function submitConfigDemo(originalBlock, modifiedBlock, Channel) {
    ...
    shell.exec(`configtxlator compute_update --channel_id ${Channel.channelName} --original ${NETWORK_PATH}/channel-artifacts/${Channel.channelName}}/${originalBlock}.pb --updated ${NETWORK_PATH}/channel-artifacts/${Channel.channelName}}/${modifiedBlock}.pb --output config_update.pb`)
...
async function main() {
    ...
    await encodeJSONtoPB("modified_config.json", "channel1")
    // sixth, run the demo submit&sign
    await submitConfigDemo("config_block", "modified_config.json", Channel)
    // this is just update the channel object
    await channel1.addPeer(peerA)
    // join the peer into the channel
    await PeerJoinChannel(peerA, orderer)
}
