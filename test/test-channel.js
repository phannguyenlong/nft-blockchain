...
async function createChannel(orderer, peers) {
    let file = fs.readFileSync(__dirname + "/../test-config/configtx.yaml")
    ...
    // wrtie to file
    let filePath = __dirname + `/channel-config/${orderer.getNormalizeChannel}`
    ...
    shell.exec(`bash -c 'cd ../; ./setupChannel.sh ${orderer.getNormalizeOrg} ${orderer.ordererPort} ${orderer.getNormalizeChannel} ${peerOrgName}; cd test/; pwd'`)
